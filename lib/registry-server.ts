...
function getCategoryFromStep(step: RegistryStep): string {
  ...
  if (name.includes("slack") || name.includes("discord") || name.includes("email") || name.includes("sms")) {
  ...
  if (name.includes("upload") || name.includes("storage") || name.includes("blob")) {
    return "storage"
  }
  if (name.includes("pdf") || name.includes("qr") || name.includes("image") || name.includes("compress")) {
    return "documents"
  }
  if (name.includes("webhook") || name.includes("api") || name.includes("fetch")) {
    return "integrations"
  }
...
export function getStepDetail(name: string): StepDetail | null {
  ...
  try {
    // Read the step file
    const filePath = path.join(process.cwd(), step.files[0])
    const code = fs.readFileSync(filePath, "utf-8")

    // Extract env vars from code
    const envVarMatches = code.matchAll(/process\.env\.(\w+)/g)
    const envVars = Array.from(new Set([...envVarMatches].map((m) => m[1]))).map((varName) => ({
      name: varName,
